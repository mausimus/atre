...
ANTIC::ANTIC(CPU* cpu, RAM* ram) :
	Chip(cpu, ram), m_renderBuffer(), m_displayBuffer(), m_renderLine(), m_displayMode(),
	m_playfieldWidth(), m_scanAddress(), m_listAddress(), m_triggerDLI(), m_hScroll(),
	m_listActive(), m_lastFrameTime(), m_scanLine(), m_scanBuffer()
{
...
void ANTIC::renderMapLine()
{
	...
	const auto bytesPerLine	 = (m_playfieldWidth / pixelWidth) * bitsPerPixel / 8;
	...
	for(auto py = 0; py < pixelHeight; py++)
	{
		...
		for(auto i = 0; i < bytesPerLine; i++)
		{
			...
			for(auto b = 0; b < pixelsPerByte; b++)
			{
				...
				{
					const auto offset =
						blankWidth + i * widthPerByte + (pixelsPerByte - 1 - b) * pixelWidth + px;
...
void ANTIC::renderCharacterLine()
{
	...
	const auto charsPerLine	 = m_playfieldWidth / charWidth;
	...
	for(auto charLine = 0; charLine < 8; charLine++)
	{
		for(auto by = 0; by < bitHeight; by++)
		{
			...
			for(auto n = 0; n < charsPerLine; n++)
			{
				...
				byte_t charSlice = m_RAM->Get(characterMap + (charNo * 8) + charLine);
				...
				for(auto bitNo = 0; bitNo < 8; bitNo++)
				{
					...
					for(auto bx = 0; bx < bitWidth; bx++)
					{
						const auto lineOffset =
							hScrollAmount + n * charWidth + (7 - bitNo) * bitWidth + bx;
...
void ANTIC::Tick()
{
	...
	if(m_scanBuffer.scanCycle == CYCLES_PER_SCANLINE)
	{
		...
		else if(m_scanLine == TOTAL_SCANLINES)
		{
			...
			const chrono::duration<double> frameTime =
				chrono::steady_clock::now() - m_lastFrameTime;
		...
		if(m_scanLine < VBLANK_SCANLINE && m_RAM->DirectGet(ChipRegisters::DMACTL) & 0b1000)
		{
			...
			m_RAM->DirectSet(ChipRegisters::GRAFM,
							 m_RAM->DirectGet(pmGraphicsBase + sectionLength * 3 + sectionOffset));
			m_RAM->DirectSet(ChipRegisters::GRAFP0,
							 m_RAM->DirectGet(pmGraphicsBase + sectionLength * 4 + sectionOffset));
			m_RAM->DirectSet(ChipRegisters::GRAFP1,
							 m_RAM->DirectGet(pmGraphicsBase + sectionLength * 5 + sectionOffset));
			m_RAM->DirectSet(ChipRegisters::GRAFP2,
							 m_RAM->DirectGet(pmGraphicsBase + sectionLength * 6 + sectionOffset));
			m_RAM->DirectSet(ChipRegisters::GRAFP3,
							 m_RAM->DirectGet(pmGraphicsBase + sectionLength * 7 + sectionOffset));
		}
	}
}
} // namespace atre
