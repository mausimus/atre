...
namespace atre
{
map<SDL_Scancode, byte_t> IO::s_scanCodes = {{SDL_SCANCODE_A, 0x3F},
											 {SDL_SCANCODE_B, 0x15},
											 {SDL_SCANCODE_C, 0x12},
											 {SDL_SCANCODE_D, 0x3A},
											 {SDL_SCANCODE_E, 0x2A},
...
IO::IO(CPU* cpu, RAM* ram) :
	IOBase(cpu, ram), m_ANTIC(cpu, ram), m_GTIA(cpu, ram, m_ANTIC.getScanBuffer()),
	m_POKEY(cpu, ram), m_PIA(cpu, ram), m_window(), m_renderer(), m_texture()
{}
		...
		if(SDL_PollEvent(&e))
		{
			...
			case SDL_KEYUP:
			{
				...
				else
				{
					...
					{
						m_POKEY.KeyDown(scanCode->second, shiftStatus, ctrlStatus);
					}
